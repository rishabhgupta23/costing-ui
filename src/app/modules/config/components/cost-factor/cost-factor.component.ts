...
import {
  SortIcons,
  TableActions
} from 'src/app/shared/constants/table.constants';
...
import {
  COSTFACTOR_TABLE_COLUMNS
} from 'src/app/data/constants/config-table.constant';
...
export class CostFactorComponent {
  ...
  factorType: CostFactorType | null = null;

  costFactorTypes = Object.values(CostFactorType);
  ...
  sortState: SortState = {
    sortColumn: 'factorName',
    sortState: SortIcons.ASC
  };
  ...
  submitCostFactorForm(): void {
    ...
    const trimmedFactorName =
      this.factorName.trim();

    if (!trimmedFactorName || !this.factorType) {
      return;
    }

    this.costFactorService.createCostFactor(
      trimmedFactorName,
      this.factorType
    ).subscribe({
    ...
    dialogRef.afterClosed().subscribe(res => {
      ...
      if (res) {
        ...
        this.costFactorService
          .updateCostFactor(row.id, res)
          .subscribe(() => {

            this.snackbarService.success(
              'Cost Factor updated successfully!'
            );
  ...
  openDeleteDialog(row: any): void {
    ...
    const dialogData: ConfirmDialogData = {
      ...
      message:
        'Are you sure you want to delete this Cost Factor?'
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (
        ...
        this.costFactorService
          .deleteCostFactor(row.id)
          .subscribe({

            next: () => {

              this.snackbarService.success(
                'Cost Factor deleted successfully!'
              );
